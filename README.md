...
- **角色扮演**：多角色卡（支持 SillyTavern 导入）、独立记忆/日记/世界书/亲密度/养成数值（角色间完全隔离）、**角色卡库全局共享**（跨对话/跨预设切换）、扮演模式（默认/快速/深度）
...
- **亲密度系统**：好感 / 信任 / 心动 三轴 + 男友力 + 里程碑（m1→m8），AI 判断加减、联动升级；进度难度（慢热/正常/快速）自选、重复加成自动递减、久别自动掉信任
