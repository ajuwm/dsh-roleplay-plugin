        ...
        slots.inject('conversation.input.overlay', function () {
          ...
          var importPngCard = function () {
            pickFile('image/png', function (dataUrl) {
              ...
              rpc('card-import-png', Object.assign(sessionId ? { sessionId: sessionId } : {}, { base64: base64 }))
                .then(function (result) {
                  var v = unwrap(result)
                  libMsgShow(v && v.ok ? v.message : ((v && v.message) || 'PNG 卡导入失败'))
                  if (sth.refresh) sth.refresh()
                  window.setTimeout(function () { if (cards.refresh) cards.refresh() }, 800)
                })
                .catch(function () { libMsgShow('PNG 卡导入失败') })
            })
          }
          var exportPngCard = function () {
            var chosen = cardSel[0]
            var hit = (cards.cards.list || []).filter(function (x) { return String(x.id) === String(chosen) })[0]
            var name = hit ? hit.name : (chosen && String(chosen).indexOf('card-') !== 0 ? chosen : (st && st.character && st.character.name) || '')
            if (!name) { libMsgShow('先选一张角色卡'); return }
            rpc('card-export-png', Object.assign(sessionId ? { sessionId: sessionId } : {}, { card: name }))
              .then(function (result) {
                var v = unwrap(result)
                ...
                libMsgShow('已导出 ' + (v.name || name) + '.png ✓')
              })
              ...
              return React.createElement('div', { className: 'rp-sb' },
                ...
                React.createElement('div', { className: 'rp-sb-body' },
                  ...
                  React.createElement('div', { className: 'rp-fold', key: 'lib' },
                    React.createElement('div', { className: 'rp-fold-head', onClick: function () { libOpen[1](!libOpen[0]); if (!lore[0].loaded) loreLoad() } },
                      React.createElement('span', null,
                    ...
                    libOpen[0] ? React.createElement('div', null,
                      React.createElement('div', { className: 'rp-lib-tabs' },
                        [['card', '人物卡'], ['lore', '世界书'], ['preset', '预设'], ['script', '剧本']].map(function (tb) {
                          return React.createElement('button', {
                            ...
                            key: tb[0],
                            onClick: function () { libTab[1](tb[0]); if (tb[0] === 'lore') loreLoad() },
                          }, tb[1])
                        })),
                      libTab[0] === 'card' ? React.createElement('div', { className: 'rp-lib-pane' },
                        React.createElement('div', { className: 'rp-sb-card-row' },
                          React.createElement('button', { className: 'rp-sb-card-switch', onClick: importPngCard }, '📥 导入 PNG 卡'),
                          React.createElement('button', { className: 'rp-sb-card-switch', onClick: exportPngCard }, '📤 导出选中卡')),
                        pngPrev[0] ? React.createElement('img', { className: 'rp-lib-pngprev', src: pngPrev[0], alt: '导出预览' }) : null,
                        React.createElement('div', { className: 'rp-sb-note-hint' }, '导入：SillyTavern 的角色卡 PNG（chara_card_v2）→ 直接开演。导出：把当前选中卡打包成 PNG。')
                      ) : null,
                      libTab[0] === 'lore' ? React.createElement('div', { className: 'rp-lib-pane' },
                        ...
                        lore[0].entries && lore[0].entries.length
                          ? lore[0].entries.map(function (e) {
                              return React.createElement('div', { className: 'rp-lore-item' + (e.constant ? ' rp-lore-const' : ''), key: e.id },
                                React.createElement('div', { className: 'rp-lore-head' },
                                  React.createElement('span', { className: 'rp-lore-keys' }, (e.constant ? '⭐ 常驻 · ' : '') + (e.keywords && e.keywords.length ? e.keywords.slice(0, 4).join(' / ') : '无关键词')),
                          ...
                          : React.createElement('div', { className: 'rp-sb-empty' }, lore[0].loaded ? '还没有世界书条目。可用「世界书」工具添加，或导入 ST 世界书 JSON。' : '加载中…')
                      ) : null,
                      libTab[0] === 'preset' ? React.createElement('div', { className: 'rp-lib-pane' },
                        ...
                        (st && st.presets && st.presets.length)
                          ? st.presets.map(function (p) {
                              return React.createElement('div', { className: 'rp-lore-item', key: p.id },
                                React.createElement('div', { className: 'rp-lore-head' },
                                  React.createElement('span', { className: 'rp-lore-keys' }, p.name + (p.enabled ? '（已启用）' : '')),
                        ...
                        React.createElement('div', { className: 'rp-lib-warn' }, '⚠️ 外部预设会作为系统提示注入，可能影响角色行为——风险自负，默认不启用。')
                      ) : null,
