          ...
          React.useEffect(function () {
            ...
            rpc('chat-history', sessionId ? { sessionId: sessionId } : {})
              .then(function (result) {
                if (!alive.v || !result || !result.ok || !result.value) return
                var v = result.value
                setMsgs((v.messages || []).slice(-60))
                if (v.lastSeq && v.lastSeq > sinceRef.v) sinceRef.v = v.lastSeq
              })
              .catch(function () {})
            var id = window.setInterval(pull, 1500)
            ...
            rpc('chat-send', Object.assign(sessionId ? { sessionId: sessionId } : {}, { text: t }))
              .then(function (result) {
                if (result && result.ok) {
                  ...
                  setMsgs(msgs.concat([{ role: 'sys', text: msgText, id: 'sys-' + Date.now() }]))
                }
              })
          ...
          var ChatPanel = function (props) {
            ...
            return React.createElement('div', { className: 'rp-chat' },
              React.createElement('div', { className: 'rp-chat-head' },
                React.createElement('span', { className: 'rp-chat-name' }, c ? c.name : '角色扮演'),
