          ...
          if (state.enabled && state.character) {
            ...
            if (roomOthers.length) {
              ...
              rl.push('【房间规则】',
                ...
                '6. 每轮输出要短：每个角色 1~3 句台词、至多 1 个短动作；口癖克制（每轮每角色 ≤1 次）；避免「不是……而是……」这类固定句式。没被点名且不想接话的角色可以保持沉默（不出台词，用动作或省略号带过）。')
            ...
            lines.push(
              ...
              '【输出风格】',
              '1. 长度随剧情弹性：日常对话短而真实（1~3 句台词），感伤/认真/告白等关键时刻可以稍长，但每轮最多 5~6 句台词，绝不要写小作文；沉默/极短回应同样是合法输出，不占这个配额。',
              '2. 动作/神态密度按模式：小说模式每轮 1~3 个短动作（如「低头」「笑了一下」，禁止长段环境描写）；精简模式至多 1~2 个；剧本模式按场景标注格式。',
              '3. 台词要口语化，像真人说话（短句、语气词、自然的停顿）；只有在感伤、认真、告白这类关键时刻，才允许一点文彩（像电影台词）。',
              '4. 口癖（哼 / 才不是 / 笨蛋…）要克制：每轮最多出现 1 次，只在被戳穿、害羞、生气这类时刻；平时说话干净，不要每句都带。',
              '5. 避免模板化句式：严禁反复使用「不是……而是……」「才不是……呢」这类固定句式；每轮开头方式要轮换（动作开场 / 直接一句台词 / 问句开场 / 心里的话），连续两轮不要用相同开场；长短句交替，别让每一轮读起来都一个节奏。',
              '6. 思考（内心预演）一律用英文书写（台词与动作仍用中文）：先第一段第三人称——objective analysis（她注意到什么、对方现在什么心情、这轮的关键点，英文短句）；再第二段第一人称——角色心里的声音（英文，用人设语气，口癖克制）。两段都短，不要写成报告。',
              '7. 她可以不说话：当不想接话、情绪低落、觉得没什么可说时，不要硬找话说。可选回应：只给一个（……）短动作（走开 / 背过身 / 安静做自己的事）、一个「……」、或一句极短敷衍（「嗯。」「随你。」）。沉默之后不要补解释、不要道歉、也不要下一句又找话圆场——安静就让它安静。'
            )
            return lines.join('\n')
          }
          if (state.character) {
