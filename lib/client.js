      ...
      apply: function (ctx) {
        ...
        function useChat(targetId) {
          ...
          var applyPoll = function (result) {
            if (!aliveBox.v || !result || !result.ok || !result.value) return
            var v = result.value
            var fresh = (v.messages || []).filter(function (m) { return m.seq > sinceRef.v })
            if (v.lastSeq && v.lastSeq > sinceRef.v) sinceRef.v = v.lastSeq
            if (fresh.length) {
              msgsBox.v = msgsBox.v.concat(fresh)
              setMsgs(msgsBox.v)
              setBusy(false)
            }
          }
          React.useEffect(function () {
            ...
            rpc('chat-history', { target: targetId })
              .then(function (result) {
                if (!aliveBox.v || !result || !result.ok || !result.value) return
                var v = result.value
                msgsBox.v = (v.messages || []).slice(-60)
                if (v.lastSeq && v.lastSeq > sinceRef.v) sinceRef.v = v.lastSeq
                setMsgs(msgsBox.v)
              })
              .catch(function () {})
            var id = window.setInterval(function () {
              rpc('chat-poll', { target: targetId, since: sinceRef.v })
                .then(applyPoll)
                .catch(function () {})
            }, 1500)
            return function () { aliveBox.v = false; window.clearInterval(id) }
          }, [targetId])
            ...
            rpc('chat-send', { target: targetId, text: t })
              .then(function (result) {
                if (result && result.ok) {
                  ...
                  msgsBox.v = msgsBox.v.concat([{ role: 'sys', text: msgText, id: 'sys-' + Date.now() }])
                  setMsgs(msgsBox.v)
                }
              })
              .catch(function () { setBusy(false) })
          }
          return { msgs: msgs, send: send, busy: busy }
        }
            ...
            var loadTargets = function () {
              rpc('chat-targets', {})
                .then(function (result) {
                  ...
                  var sel = null
                  if (chatTarget) sel = list.filter(function (x) { return x.sessionId === chatTarget })[0] || null
                  if (!sel) sel = list[0]   // 已按最近活跃排序
