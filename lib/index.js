  ...
  apply(ctx) {
    ...
    async function dispatch(endpoint, payload) {
      ...
      if (endpoint === 'chat-send') {
        const value = await face.chatSend({ sessionId, text: payload.text })
        return { ok: !!value.ok, value }
      }
      if (endpoint === 'chat-poll') {
        const value = await face.chatPoll({ sessionId, since: payload.since })
        return { ok: true, value }
      }
      if (endpoint === 'chat-history') {
        const value = await face.chatHistory({ sessionId, limit: payload.limit })
        return { ok: true, value }
      }
