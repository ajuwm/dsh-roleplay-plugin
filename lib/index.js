...
export default {
  ...
  apply(ctx) {
    ...
    async function dispatch(endpoint, payload) {
      ...
      const KNOWN_ENDPOINTS = ['settings-read', 'settings-write', 'chat-targets', 'chat-send', 'chat-poll', 'chat-history', 'notes-list', 'notes-ack', 'game-start', 'game-move', 'game-state', 'game-quit', 'backup-now', 'card-import-png', 'card-export-png', 'preset-list', 'preset-import', 'preset-remove', 'preset-set-enabled', 'portrait-list', 'lore-list', 'lore-import', 'lore-remove', 'get-state', 'start', 'stop', 'cards-list', 'cards-load', 'look-desktop', 'shop-buy', 'inventory-use', 'settings-update', 'user-profile-update', 'cards-delete', 'room-start', 'room-stop', 'pet-status', 'pet-start', 'pet-stop']
      ...
      // ST 生态: PNG 卡导入导出 / 预设管理 / 世界书 / 画像素描
      if (endpoint === 'card-import-png' || endpoint === 'card-export-png' || endpoint === 'preset-list' || endpoint === 'preset-import' || endpoint === 'preset-remove' || endpoint === 'preset-set-enabled' || endpoint === 'portrait-list' || endpoint === 'lore-list' || endpoint === 'lore-import' || endpoint === 'lore-remove') {
        const sid = payload.target !== undefined ? String(payload.target) : (payload.sessionId !== undefined ? String(payload.sessionId) : undefined)
        const targetFace = resolveFace(sid)
        if (targetFace === undefined) {
          return { ok: false, error: { code: 'roleplay-unavailable', message: '目标角色扮演会话不可用。' } }
        ...
        if (endpoint === 'card-import-png') {
          const value = await targetFace.cardImportPng({ sessionId: sid, base64: payload.base64 })
          return { ok: !!value.ok, value }
        }
