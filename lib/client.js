  ...
  factory: function (require) {
    ...
    var plugin = {
      ...
      apply: function (ctx) {
        ...
        // 对话侧边栏消息流:历史 + 增量轮询 + 发送(经 /roleplay/chat-* 端点)。
        function useChat(sessionId) {
          ...
          // 游标与消息盒随会话 id 重置(多角色扮演会话并存时,侧栏跟随当前活跃会话切换)
          var sinceRef = { v: 0 }
          var msgsBox = { v: [] }
          var aliveBox = { v: true }
          ...
          React.useEffect(function () {
            ...
            setMsgs([])
            setBusy(false)
            rpc('chat-history', sessionId ? { sessionId: sessionId } : {})
              .then(function (result) {
                if (!aliveBox.v || !result || !result.ok || !result.value) return
                var v = result.value
            ...
            var id = window.setInterval(function () {
              rpc('chat-poll', Object.assign(sessionId ? { sessionId: sessionId } : {}, { since: sinceRef.v }))
                .then(applyPoll)
                .catch(function () {})
            }, 1500)
          ...
          var send = function (text) {
            ...
            if (!t) return
            setBusy(true)
            rpc('chat-send', Object.assign(sessionId ? { sessionId: sessionId } : {}, { text: t }))
              .then(function (result) {
                if (result && result.ok) {
            ...
            function (props) {
              ...
              return React.createElement('div', { className: 'rp-dock' },
                React.createElement('button', {
                  ...
                  title: active ? ('💬 和 ' + st.character.name + ' 对话') : '角色扮演对话(需先开演)',
                },
                  React.createElement('span', null, '💬'),
        ...
        slots.inject('conversation.input.overlay', function () {
          ...
          // ── 对话侧边栏:与角色侧栏并排的独立聊天窗口 ──
          var ChatPanel = function (props) {
            var sessionId = props && props.sessionId ? props.sessionId : undefined
            var chat = useChat(sessionId)
            var sth = useRoleplayState(sessionId)
            var st = sth.st
            var c = st && st.enabled && st.character ? st.character : null
            ...
            return React.createElement('div', { className: 'rp-chat' },
              React.createElement('div', { className: 'rp-chat-head' },
                React.createElement('span', { className: 'rp-chat-name', title: '与「当前活跃会话」中挂载的角色扮演角色对话；切换会话时自动跟随' }, c ? c.name : '角色扮演'),
                c ? React.createElement('span', { className: 'rp-chat-sub' }, '跟随当前对话') : null,
                React.createElement('span', { className: 'rp-chat-dot' + (c ? '' : ' rp-off') }),
              ...
              React.createElement('div', { className: 'rp-chat-body', ref: function (el) { bodyEl.current = el } },
                !c ? React.createElement('div', { className: 'rp-chat-hint' }, '尚未开始角色扮演——先在主界面或角色侧栏开演后再聊。\n仍可查看此会话的历史对话。') : null,
                msgs.map(function (m) {
                  var cls = 'rp-chat-msg'
              ...
              React.createElement('div', { className: 'rp-chat-input-wrap' },
                React.createElement('input', {
                  ...
                  value: input,
                  placeholder: c ? '和' + c.name + '说点什么…' : '尚未开演',
                  disabled: !c,
                  onChange: function (e) { setInput(e.target.value) },
                  onKeyDown: function (e) { if (e.key === 'Enter') doSend() },
                }),
                React.createElement('button', { className: 'rp-chat-send', disabled: !c || !input.trim(), onClick: doSend }, '发送')))
          }
          slots.inject('conversation.input.overlay', function () {
            return slots.register(
              ...
              function (props) {
                ...
                React.useEffect(function () { return subscribeChat(function (v) { setLocal(v) }) }, [])
                if (!open) return null
                return React.createElement(ChatPanel, { sessionId: props && props.sessionId })
              }
            )
          })
