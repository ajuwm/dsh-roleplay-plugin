    ...
    var plugin = {
      ...
      apply: function (ctx) {
        ...
        style.textContent = [
          ...
          // ── 悬浮宠物插件(dsh-whale-musume 等)会以 z-index 1199(气泡)/60(本体) 盖在
          //    DSH composer 容器(z:7 sticky 上下文)之上 —— 🎭/💬 dock 按钮被吞点击,
          //    侧栏打不开。把 composer 容器抬到 1300(容器透明, 宠物本体照常显示),
          //    按钮恢复可点(canvas 实测: 注入后元素栈首位即 rp-dock-btn)。
          'body [class*="composerSeat"] { z-index: 1300 !important; }',
          // ── DSH「插件配置」→「角色扮演」卡片 ──
