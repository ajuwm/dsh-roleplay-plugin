...
import { needMirror, pickSnapshotDir, restoreSourceOf } from '../agent-presets/roleplay/lib/backup-core.mjs'

const PKG_VERSION = '1.5.23'
const PRESET_NAMES = ['roleplay', 'roleplay-friend', 'roleplay-oc']
// L4 用户级镜像根: 跨 workspace(默认 %DSH_HOME%\.roleplay-backups; DSH_ROLEPLAY_BACKUP_DIR 可覆盖)。
// 与 materializePresets 同一 DSH_HOME 解析, 用户级目录耐删(删除工作区不影响)。
const USER_BACKUP_DIR = process.env.DSH_ROLEPLAY_BACKUP_DIR || path.join(process.env.DSH_HOME || path.join(os.homedir(), '.dsh'), '.roleplay-backups')
const DATA_ROOTS = ['.roleplay', '.roleplay-friend', '.roleplay-oc']
...
function materializePresets() {
  try {
    ...
    // 全新 DSH_HOME 里 .agent-presets 还不存在 —— 必须自己建, 否则老实现直接 return,
    // 导致新装用户永远拿不到任何扮演预设(实测: 干净 home 下预设目录始终为空)。
...
export default {
  ...
  apply(ctx) {
    ...
    async function dispatch(endpoint, payload) {
      ...
      // 未知端点优先报 bad-endpoint(避免被后续 face 检查误报为 roleplay-unavailable)
      const KNOWN_ENDPOINTS = ['settings-read', 'settings-write', 'chat-targets', 'chat-send', 'chat-poll', 'chat-history', 'notes-list', 'notes-ack', 'game-start', 'game-move', 'game-state', 'game-quit', 'backup-now', 'card-import-png', 'card-export-png', 'card-import-json', 'card-brief', 'card-detail', 'card-art', 'card-update', 'preset-list', 'preset-import', 'preset-remove', 'preset-set-enabled', 'preset-items', 'preset-set-item', 'portrait-list', 'lore-list', 'lore-import', 'lore-remove', 'lore-add', 'lore-update', 'get-state', 'start', 'stop', 'cards-list', 'cards-load', 'look-desktop', 'shop-buy', 'inventory-use', 'settings-update', 'user-profile-update', 'cards-delete', 'room-start', 'room-stop', 'pet-status', 'pet-start', 'pet-stop']
      ...
      if (endpoint === 'chat-targets') {
        ...
        try {
          ...
          for (const a of roots) {
            ...
            const info = await f.peek({ sessionId: String(a.id) })
      ...
      if (endpoint === 'backup-now') {
        const sid = payload.target !== undefined ? String(payload.target) : (payload.sessionId !== undefined ? String(payload.sessionId) : undefined)
        const targetFace = resolveFace(sid)
        if (targetFace === undefined || typeof targetFace.backupNow !== 'function') {
          return { ok: false, error: { code: 'roleplay-unavailable', message: '目标角色扮演会话不可用。' } }
        }
        const value = await targetFace.backupNow({ sessionId: sid })
        return { ok: !!value.ok, value }
      }
      // ST 生态: 卡导入导出/卡库/预设条目/世界书/画像
      if (endpoint === 'card-import-png' || endpoint === 'card-export-png' || endpoint === 'card-import-json' || endpoint === 'card-brief' || endpoint === 'card-detail' || endpoint === 'card-art' || endpoint === 'card-update' || endpoint === 'preset-list' || endpoint === 'preset-import' || endpoint === 'preset-remove' || endpoint === 'preset-set-enabled' || endpoint === 'preset-items' || endpoint === 'preset-set-item' || endpoint === 'portrait-list' || endpoint === 'lore-list' || endpoint === 'lore-import' || endpoint === 'lore-remove' || endpoint === 'lore-add' || endpoint === 'lore-update') {
        const sid = payload.target !== undefined ? String(payload.target) : (payload.sessionId !== undefined ? String(payload.sessionId) : undefined)
        const targetFace = resolveFace(sid)
        if (targetFace === undefined) {
          return { ok: false, error: { code: 'roleplay-unavailable', message: '目标角色扮演会话不可用。' } }
        }
        if (endpoint === 'card-import-png') {
          const value = await targetFace.cardImportPng({ sessionId: sid, base64: payload.base64, overwrite: payload.overwrite, saveAsName: payload.saveAsName })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'card-import-json') {
          const value = await targetFace.cardImportJson({ sessionId: sid, json: payload.json, overwrite: payload.overwrite, saveAsName: payload.saveAsName })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'card-export-png') {
          const value = await targetFace.cardExportPng({ sessionId: sid, card: payload.card })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'card-brief') return { ok: true, value: await targetFace.cardBrief({ sessionId: sid }) }
        if (endpoint === 'card-detail') return { ok: true, value: await targetFace.cardDetail({ sessionId: sid, card: payload.card }) }
        if (endpoint === 'card-art') return { ok: true, value: await targetFace.cardArt({ sessionId: sid, card: payload.card, full: payload.full }) }
        if (endpoint === 'card-update') {
          const value = await targetFace.cardUpdate({ sessionId: sid, id: payload.id, name: payload.name, fields: payload.fields })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'preset-list') return { ok: true, value: await targetFace.presetList({ sessionId: sid }) }
        if (endpoint === 'preset-items') {
          const value = await targetFace.presetItems({ sessionId: sid, id: payload.id })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'preset-set-item') {
          const value = await targetFace.presetSetItem({ sessionId: sid, id: payload.id, index: payload.index, enabled: payload.enabled })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'preset-import') {
          const value = await targetFace.presetImport({ sessionId: sid, json: payload.json, filename: payload.filename })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'preset-remove') {
          const value = await targetFace.presetRemove({ sessionId: sid, id: payload.id })
          return { ok: true, value }
        }
        if (endpoint === 'preset-set-enabled') {
          const value = await targetFace.presetSetEnabled({ sessionId: sid, id: payload.id, enabled: payload.enabled })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'lore-list') return { ok: true, value: await targetFace.loreList({ sessionId: sid }) }
        if (endpoint === 'lore-import') {
          const value = await targetFace.loreImport({ sessionId: sid, json: payload.json })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'lore-add') {
          const value = await targetFace.loreAdd({ sessionId: sid, keywords: payload.keywords, content: payload.content, priority: payload.priority, constant: payload.constant })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'lore-update') {
          const value = await targetFace.loreUpdate({ sessionId: sid, id: payload.id, fields: payload.fields })
          return { ok: !!value.ok, value }
        }
        if (endpoint === 'lore-remove') {
          const value = await targetFace.loreRemove({ sessionId: sid, id: payload.id })
          return { ok: true, value }
        }
        return { ok: true, value: await targetFace.portraitList({ sessionId: sid }) }
      }
      ...
      if (endpoint === 'pet-status' || endpoint === 'pet-start' || endpoint === 'pet-stop') {
        // 1.5.15: 桌宠行已从「恋爱向」预设组合树移出(预设挂载被其拖垮);
        // 桥接先读宿主 ctx 上的 deskpet 服务(未来 host 平面挂载用), 回退按预设子树解析。
        let pet = undefined
        try { pet = ctx.get('deskpet') } catch (e) { /* 未就绪 */ }
        if (pet === undefined) pet = agent !== undefined ? agentPresets.serviceFor(agent, 'deskpet') : undefined
        if (pet === undefined) {
          return {
            ok: false,
            error: {
    ...
    // ── L4/L5 用户级备份镜像: 引擎 L2 快照 → %DSH_HOME%\.roleplay-backups(跨 workspace 耐删);
    //    数据根缺失/为空 → 从最新镜像回流(Node fs 不受工作区沙盒限制, 与 materializePresets 同理)。
    function dirDates(p) {
      try { return fs.readdirSync(p).filter((x) => /^\d{4}-\d{2}-\d{2}$/.test(x)).sort() } catch (e) { return [] }
    }
    async function mirrorBackups() {
      let sp = undefined
      try { sp = ctx.get('sandboxPolicy') } catch (e) { /* 未就绪 */ }
      const wr = sp && sp.workspaceRoot ? sp.workspaceRoot : null
      if (!wr) return
      for (const dr of DATA_ROOTS) {
        try {
          const localRoot = path.join(wr, '.roleplay-backup', dr)
          const remoteRoot = path.join(USER_BACKUP_DIR, dr)
          const localDates = dirDates(localRoot)
          if (!localDates.length) continue
          const remoteDates = dirDates(remoteRoot)
          // L4 增量镜像: 本工作区有、远端还没有的日期
          for (const d of needMirror(localDates, remoteDates)) {
            const s = path.join(localRoot, d)
            const t = path.join(remoteRoot, d)
            fs.mkdirSync(t, { recursive: true })
            for (const f of fs.readdirSync(s)) {
              const sf = path.join(s, f)
              if (fs.existsSync(sf) && fs.statSync(sf).isFile()) fs.copyFileSync(sf, path.join(t, f))
            }
          }
          // 远端与本工作区均只留最近 3 份
          for (const d of pickSnapshotDir(dirDates(remoteRoot), 3).drop) fs.rmSync(path.join(remoteRoot, d), { recursive: true, force: true })
          for (const d of pickSnapshotDir(dirDates(localRoot), 3).drop) fs.rmSync(path.join(localRoot, d), { recursive: true, force: true })
          // L5 灾后回流: 数据根无有效文件且镜像存在 → 整目录补回
          const dataDir = path.join(wr, dr)
          let files = []
          try { files = fs.readdirSync(dataDir) } catch (e) { files = [] }
          const src = restoreSourceOf(dirDates(remoteRoot), files)
          if (src) {
            for (const f of fs.readdirSync(path.join(remoteRoot, src))) {
              const sf = path.join(remoteRoot, src, f)
              const tf = path.join(dataDir, f)
              if (fs.existsSync(sf) && fs.statSync(sf).isFile()) {
                fs.mkdirSync(path.dirname(tf), { recursive: true })
                fs.copyFileSync(sf, tf)
              }
            }
            console.error('[roleplay-backup] 已回流数据根 ' + dr + ' ← ' + src)
          }
        } catch (e) { console.error('[roleplay-backup] mirror error (' + dr + ')', e) }
      }
    }
    let timerSvc = undefined
    try { timerSvc = ctx.get('timer') } catch (e) { /* 未就绪 */ }
    if (timerSvc !== undefined && typeof timerSvc.interval === 'function') {
      timerSvc.interval(() => { mirrorBackups().catch((e) => console.error('[roleplay-backup] mirror failed', e)) }, 60000)
    } else {
      console.error('[roleplay-client] backup mirror disabled: no timer service')
    }

    const route = {
      kind: 'prefix',
      path: '/roleplay',      handler: async (req, res) => {
